...
(defhydra ans/buffer-window-hydra (:exit t :hint nil)
  ...
  Switch:  _h__j__k__l_  Move (far = g): _H__J__K__L_    Misc: _r_evert   _s_ave as   _z_:undo   _Z_:redo   _SPC_: Last"
  ("w" quit-window)
...
(use-package ess
  ...
  (general-def
    ...
    "p p" 'ans/r-send-function-or-paragraph
    "p d" 'ans/r-send-function-or-para-and-down
...
(defun ans/r-send-region-source (start end)
  "Use R source function to run region from START to END."
  (interactive "r")
  (let* ((temp-file (make-temp-file "/tmp/")))
    (write-region start end temp-file)
    (ess-send-string (ess-get-process) (format "source('%s', echo = TRUE)" temp-file))))

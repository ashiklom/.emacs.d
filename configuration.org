...
Menu bar mode is required for Emacs Macport to work correctly with Mission Control, etc.
See [[https://github.com/railwaycat/homebrew-emacsmacport/issues/124][this issue]].
...
Get the executable path from the current shell.
Currently disabled because (a) it's slow, and (b) it doesn't work well with TRAMP.
...
#+END_SRC
**** Custom surround macro

My attempt at adding support for Markdown.
...
(define-and-bind-quoted-text-object "underscore" "*" "*" "*")
(add-to-list 'evil-surround-pairs-alist '(?_ . ("*" . "*")))
#+END_SRC
*** evil-embrace
#+BEGIN_SRC emacs-lisp
...
(use-package evil-mc
  ...
  (general-def
    ...
    "grz" #'evil-mc-undo-last-added-cursor
...
(defhydra ans/buffer-window-hydra (:exit t :hint nil)
  ...
  Switch:  _h__j__k__l_  Move (far = g): _H__J__K__L_    Misc: _u_ revert   _s_ave as   _z_:undo   _Z_:redo   _SPC_:Last  _r_:Rotate  _o_:Ace window"
  ("w" quit-window)
...
(use-package dired
  ...
  (general-def
    :keymaps 'dired-mode-map
    :states 'normal
    "ge" #'wdired-change-to-wdired-mode)
  (defun ans/dired-mode-config ()
    "Custom hooks for dired mode."
    (interactive)
    (let ((inhibit-message t))
      (toggle-truncate-lines 1)))
  (add-hook 'dired-mode-hook #'ans/dired-mode-config))
#+END_SRC
...
(use-package perspective
  :ensure t
  :init
  (setq persp-mode-prefix-key (kbd "SPC TAB"))
  :config
  (persp-mode)
  :general
  (general-def
...
(use-package csv-mode
  ...
  (general-def
    ...
    "C-<right>" #'csv-forward-field
    "C-<left>" #'csv-backward-field)
...
Don't highlight mouse selections in the agenda.
(Note that clicking still works.)
...
(use-package ess
  ...
  (general-def
    ...
    :states 'normal
    "C-RET" #'markdown-follow-link-at-point)
  (general-def
    :states 'insert
    :keymaps 'ess-mode-map
    "_" 'self-insert-command
...
(defun ans/ess-dev-off ()
  ...
  (ess-send-string
   ...
   "dev.off()"))
#+END_SRC
*** Split path into comma-separate list

Quickly convert code like ~file.path("path/to/file")~ to ~file.path("path", "to", "file")~.
...
Most of the code I usually write (R, C) doesn't have a natural concept for sexps, which causes certain bugs (like ~sp-forward-slurp-sexp~ incorrectly including commas).
"Hybrid" sexps provide a workaround.
See the [[https://github.com/Fuco1/smartparens/wiki/Hybrid-S-expressions][smartparens wiki]] for more information.

Note: A better solution (for R, at least) may be to fix the regular expression for ESS sexps (~sp-sexp-prefix/suffix~).

#+BEGIN_SRC emacs-lisp
(defvar ans/sp-nonhybrid-sexp-modes
  '(emacs-lisp-mode
    lisp-interaction-mode
    common-lisp-mode
    clojure-mode
    scheme-mode)
  "List of major modes for which to use regular sexps instead of hybrid sexps.
Typically, these are all members of the lisp family.")

(defun ans/sp-slurp-and-end ()
  "Slurp up sexp and move to its end."
  (interactive)
  (if (member major-mode ans/sp-nonhybrid-sexp-modes)

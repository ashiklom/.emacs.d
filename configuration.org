...
Note that instead of the default ~org-mode~, I use ~org-plus-contrib~, which has some additional contributed features.
The one I'm currently interested in is ~ignore-headlines~, which allows me to make certain headlines invisible upon export.
This is useful for leveraging org mode organizational features while exporting simplified documents.
